...
WITH listing_distances AS (
	SELECT 
        ...
        (pop_m_total_2km_weighted + pop_f_total_2km_weighted) AS total_weighted_pop,
	...
	FROM (
        SELECT
            ...
            ROUND(v.pop_m_total_2km * CAST(%(m)s AS numeric) * 0.025) AS pop_m_total_2km_weighted,  -- Replaced by the m parameter
            ROUND(v.pop_f_total_2km * (1 - CAST(%(m)s AS numeric)) * 0.025) AS pop_f_total_2km_weighted,  -- Replaced by the m parameter

        v.* -- The rest of the cols, in case we need them
        FROM 
            listings_pop_radius_view v
        WHERE 
            v.property_inquiry_number = %(id)s
  )
)
